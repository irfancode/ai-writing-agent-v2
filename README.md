...
### ✨ Brand Voice DNA
Upload 3 samples of your best writing → AI learns your style, vocabulary, tone → Generates content in YOUR voice.

```bash
./run.sh voice create --name mybrand --samples "Your sample text 1" "Sample 2" "Sample 3"
./run.sh voice list
```
...
```bash
./run.sh version save --doc my-article --content "Content here"
./run.sh version list --doc my-article
./run.sh version rollback --doc my-article --version-id v1_20240415
```
...
## 🛠️ Quick Start (Zero-Cost Mode - No API Keys!)
